...
// The API key is read from the ANTHROPIC_API_KEY environment variable (set in Vercel dashboard)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (status === 401) return res.status(401).json({ error: 'Invalid API key configured on server.' });
